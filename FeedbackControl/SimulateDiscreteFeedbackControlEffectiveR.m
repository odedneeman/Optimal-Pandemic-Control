clear;
addpath("..\Common\")
load("..\ContinuousTime\ContinuousSensitivityResultsWithEndo\beta_N_0.53_smoothed_discrete_guess.mat", "xList")
referenceXList = xList;
referenceDeathToll = xList(5,:);
referenceResolving = xList(4,:);
referenceSusceptible = xList(1,:);
load("..\ContinuousTime\ContinuousSensitivityResultsWithEndo\beta_N_0.53_smoothed_discrete_guess.mat", "nList")
referenceNList = nList; 
load("..\ContinuousTime\ContinuousSensitivityResultsWithEndo\beta_N_0.53_smoothed_discrete_guess.mat", "costList")
costList = costList(costList>0);
referenceCost = min(real(costList));
load("..\ContinuousTime\ContinuousSensitivityResultsWithEndo\beta_N_0.53_smoothed_discrete_guess.mat", "betaList")
referenceBetaList = betaList;
costFinalStep = 63501;
tV = 540;

paramType = 1; % 1 - betaW, 2 - betaN
fileSuff = ""; % if you want to add a suffix to the file name

betaNList = 0.4:0.0025:0.6;
betaWList = 0.3:0.002:0.45;
optionList = {betaWList, betaNList};
...
fileName = "./matResults/statsPerBeta" + betaStr + "EffectiveRControl.mat";
save(fileName, "statsPerParam");
