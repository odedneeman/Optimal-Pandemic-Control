function ret = continuousOptimizationAdam(newParams, saveFileName, loadFileName)
    ...
    for curIteration = 1:maxIter
        ...
        % Run Adam optimizer
        adamG = minimizerList - betaList;
        adamM = adamBeta1 * adamM + (1 - adamBeta1) * adamG;
        adamV = adamBeta2 * adamV + (1 - adamBeta2) * (adamG .^2);
        adamMHat = adamM/(1 - adamBeta1^(curIteration - 1));
        adamVHat = adamV/(1 - adamBeta2^(curIteration - 1));
       
        betaList = betaList + fixedStepsize * adamMHat./(adamVHat.^0.5 + adamEps);
    end
